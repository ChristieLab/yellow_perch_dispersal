...
exampleLoci<-read.vcfR("raw.maxmiss.7.missindv.7.maf.01.recode.vcf")
...
#plot results
#deviation plot
ggplot()+geom_point(data=example_output,aes(x=het_perc,y=z),alpha=0.5)+xlim(0,1)+ylim(-10,10)
#ratio plot
ggplot()+geom_point(data=example_output,aes(x=het_perc,y=ratio),alpha=0.5)+xlim(0,1)

#write.csv(example_output, file="HDplot.GLCv2.csv")


#Create a blacklist of loci flagged as possible paralogs to remove using vcftools - MAKE SURE TO CHANGE THE het_per AND z PARAMETERS BELOW
#BASED ON YOUR OUTPUT
high_het=example_output[which(example_output$het_perc>0.5),]
no_high_het=example_output[which(example_output$het_perc<0.5),]
no_hi_z=no_high_het[which(no_high_het$z>5),] ##adjusted to 5 based on output
no_low_z=no_high_het[which(no_high_het$z< -5),] ##adjusted to -5 based on output
blacklist=rbind(high_het,no_hi_z,no_low_z)
write.csv(blacklist, file="maxmiss.7.missindv.7.maf.01_HDplot_results_filtered.csv")
write.table(blacklist[,1:2],file="hdplot_blacklist_cuning2all_g70low50mac3.txt", sep="\t", quote=FALSE, col.names=FALSE, row.names=FALSE)

##15789 remaining for last step ≥≥selecting best snp on individual loci
